...
public abstract class AbstractMotorController {
    ...
    /**
     * Used for {issues} and gives a potential fix based on built in failure
     * reported issue
     *
    ...
    /**
     * Self explanatory. Takes into account {@link #getMotorTemperature() motor temp} and {
     * } to determine if the motor is running too hot for usage
     *
     * @return invert {@link #isOverheated}
     */
    protected boolean isTemperatureAcceptable() {
        if (Constants.ENABLE_OVERHEAT_DETECTION) {
            if (getMotorTemperature() >= Constants.OVERHEAT_THRESHOLD) {
                if (!isOverheated) {
                    UserInterface.smartDashboardPutBoolean("OVERHEAT " + getID(), false);
                    isOverheated = true;
                }
            } //wait 5 degrees to unoverheat
            else if (isOverheated && getMotorTemperature() < Constants.OVERHEAT_THRESHOLD - 5) {
                isOverheated = false;
